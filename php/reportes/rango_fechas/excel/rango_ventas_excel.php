...
$objSheet->setCellValue('A5', 'Boleta N°');
$objSheet->setCellValue('B5', 'Fecha');
$objSheet->setCellValue('C5', 'Cliente');
...
$c=mysql_query("SELECT A.*, B.NOMBRE_C AS NOMBRE FROM FACTURA AS A, CLIENTE_1 AS B WHERE A.RUC_CLIENTE=B.RUC_DNI AND (FECHA_FACTURA BETWEEN '$inicio' AND '$final') AND PROCESO='2'",$conexion);
$numero=5;
...
if (!isset($inicio,$final)){
$c=mysql_query("SELECT A.*, B.NOMBRE_C AS NOMBRE FROM FACTURA AS A, CLIENTE_1 AS B WHERE A.RUC_CLIENTE=B.RUC_DNI AND PROCESO='2' ",$conexion);
$numero=5;
while ($dato=mysql_fetch_array($c)) {
	$numero++;
	$objSheet->setCellValue('A'.$numero, $dato['NRO_FACTURA']);
