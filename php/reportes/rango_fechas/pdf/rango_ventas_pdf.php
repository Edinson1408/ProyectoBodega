...
$html.='<hr></>';
$html.='</div>';
$sql_rango=mysql_query("SELECT A.*, B.NOMBRE_C AS NOMBRE FROM BOLETA AS A, CLIENTE_1 AS B WHERE A.RUC_CLIENTE=B.RUC_DNI AND (FECHA_FACTURA BETWEEN '$inicio' AND '$final')",$conexion);
$html.='<table>';
$html.='<thead>';
$html.='<tr>';
$html.='<th>Boleta N°</th>';
...
if (!isset($inicio,$final)){
$sql_rango=mysql_query("SELECT A.*, B.NOMBRE_C AS NOMBRE FROM BOLETA AS A, CLIENTE_1 AS B WHERE A.RUC_CLIENTE=B.RUC_DNI",$conexion);
while ($ARR_FAC=mysql_fetch_array($sql_rango)) {
	$html.='<tbody><tr><td>'.$ARR_FAC['NRO_FACTURA'].'</td><td>'.$ARR_FAC['FECHA_FACTURA'].'</td><td>'.$ARR_FAC['NOMBRE'].'</td><td>'.$ARR_FAC['SUB_TOTAL'].'</td><td>'.$ARR_FAC['IGV'].'</td><td>'.$ARR_FAC['TOTAL'].'</td></tr></tbody>';
}
}
...
$html.='<hr></>';
if (isset($inicio,$final)){
$sql_rango=mysql_query("SELECT SUM(TOTAL) AS SUMA, COUNT(NRO_FACTURA) AS FILA FROM BOLETA WHERE (FECHA_FACTURA BETWEEN '$inicio' AND '$final')",$conexion);
while ($ARR_FAC=mysql_fetch_array($sql_rango)) {
...
if (!isset($inicio,$final)){
$sql_rango=mysql_query("SELECT SUM(TOTAL) AS SUMA, COUNT(NRO_FACTURA) AS FILA  FROM BOLETA ",$conexion);
while ($ARR_FAC=mysql_fetch_array($sql_rango)) {
