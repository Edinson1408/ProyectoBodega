...
$html.='<th>Estado</th>';
$html.='</tr>';
$SQL_FACTURA=mysql_query("SELECT a.*, b.NOMBRE_C AS CLIENTE, c.NOMBRE_ESTADO AS ESTA FROM BOLETA as a , CLIENTE_1 as b, ESTADO AS c WHERE a.RUC_CLIENTE=b.RUC_DNI AND a.ESTADO=c.ID_ESTADO AND FECHA_FACTURA='$FECHA_HOY' AND A.ID_TURNO='$ID_TURNO'",$conexion);
...
$html.='</table>';
$html.='<table class="datos">';
$SQL_TOTAL=mysql_query("SELECT SUM(TOTAL) AS TOTAL FROM BOLETA WHERE FECHA_FACTURA='$FECHA_HOY' AND ID_TURNO='$ID_TURNO' AND ESTADO='C'",$conexion);
...
$html.='<th>Total</th>';
$html.='</tr>';
$SQL_BC=mysql_query("SELECT COUNT(NRO_FACTURA) AS BC, SUM(TOTAL) AS TC FROM BOLETA WHERE ESTADO='C' and FECHA_FACTURA='$FECHA_HOY' AND ID_TURNO='$ID_TURNO'",$conexion);
...
$html.='<th>Total</th>';
$html.='</tr>';
$SQL_BP=mysql_query("SELECT COUNT(NRO_FACTURA) AS BP, SUM(TOTAL) AS TP FROM BOLETA WHERE ESTADO='P' and FECHA_FACTURA='$FECHA_HOY' AND ID_TURNO='$ID_TURNO'",$conexion);
...
$SQL_TOTAL=mysql_query("SELECT SUM(TOTAL) AS TOTAL FROM BOLETA WHERE FECHA_FACTURA='$FECHA_HOY' AND ID_TURNO='$ID_TURNO'",$conexion);
while ($ARR_TOTAL=mysql_fetch_array($SQL_TOTAL)) {
$html.='<tr>
...
$SQL_CONTADO=mysql_query("SELECT COUNT(NRO_FACTURA) AS CONTADO FROM BOLETA WHERE FECHA_FACTURA='$FECHA_HOY' AND ID_TURNO='$ID_TURNO'",$conexion);
