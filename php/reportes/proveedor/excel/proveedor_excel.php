...
$objSheet->setCellValue('A5', 'Proveedor');
$objSheet->setCellValue('B5', 'Factura N°');
...
$c=mysql_query("SELECT A.*, B.NOMBRE_CLIENTE AS NOMBRE FROM FACTURA AS A, CLIENTE AS B WHERE A.RUC_CLIENTE=B.RUC_CLIENTE AND (FECHA_FACTURA BETWEEN '$inicio' AND '$final') AND A.RUC_CLIENTE='$ruc' AND PROCESO='1'");
$numero=5;
while ($dato=mysql_fetch_array($c)) {
	$numero++;
	$objSheet->setCellValue('A'.$numero, $dato['NOMBRE']);
	$objSheet->setCellValue('B'.$numero, $dato['NRO_FACTURA']);
...
foreach(range('A', 'F') as $column){
		$objSheet->getColumnDimension($column)->setAutoSize(true)->setWidth(20);
}
...
$objXLS->getActiveSheet()->setTitle('Proveedor');
