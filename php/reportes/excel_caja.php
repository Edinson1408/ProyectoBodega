...
header('Content-Disposition: attachment; filename=DIARIO_'.$hoy.'.xls');
...
//titutlo
$objSheet->setCellValue('A4', 'COMPRAS DEL MES '.$hoy.'');
...
if (!isset($filtro)) {	
$SQL_VENTAS=mysql_query("SELECT a.*, b.NOMBRE_PRODUCTO, c.FECHA_FACTURA FROM  detalle_boleta as a, producto as b, boleta as c WHERE a.COD_PRODUCTO=b.COD_PRODUCTO AND a.NRO_FACTURA=c.NRO_FACTURA AND C.FECHA_FACTURA='$hoy' order by a.NRO_FACTURA  ",$conexion);
$numero=5;
...
if (isset($filtro)) {	
$SQL_VENTAS=mysql_query("SELECT a.*, b.NOMBRE_PRODUCTO, c.FECHA_FACTURA FROM  detalle_boleta as a, producto as b, boleta as c WHERE a.COD_PRODUCTO=b.COD_PRODUCTO AND a.NRO_FACTURA=c.NRO_FACTURA AND C.FECHA_FACTURA='$hoy' AND C.ID_TURNO='$turno' order by a.NRO_FACTURA  ",$conexion);
$numero=5;
while ($dato=mysql_fetch_array($SQL_VENTAS)) {
	$numero++;
	$objSheet->setCellValue('A'.$numero, $dato['NRO_FACTURA']);
