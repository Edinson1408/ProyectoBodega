...
$sql_detalle=mysql_query("SELECT SUM(CANTIDAD_PRODUCTO) AS TOTAL_DETA, PRECIO_UNITARIO FROM DETALLE_BOLETA WHERE NRO_FACTURA='$FACTURA' AND COD_PRODUCTO='$COD_PRODUCTO'",$conexion);
$ARRAY_DETALLE=mysql_fetch_array($sql_detalle);

$sumar=$ARRAY_DETALLE['TOTAL_DETA']+$ARRAY_ALMACEN['CANTIDAD'];
$calculo=$ARRAY_DETALLE['PRECIO_UNITARIO']*$ARRAY_DETALLE['TOTAL_DETA'];


$sql_boleta=mysql_query("SELECT * FROM BOLETA WHERE NRO_FACTURA='$FACTURA'",$conexion);
...
mysql_query("UPDATE BOLETA SET TOTAL='$RESTAR', SUB_TOTAL='$SUB_TOTAL', IGV='$IGV' WHERE NRO_FACTURA='$FACTURA'",$conexion);

mysql_query("DELETE FROM DETALLE_BOLETA WHERE NRO_FACTURA='$FACTURA' AND COD_PRODUCTO='$COD_PRODUCTO'",$conexion);
