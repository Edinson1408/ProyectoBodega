...
header('Content-Disposition: attachment; filename=MOVIMIENTO_VENTAS_'.$meses[$mes].'_'.$año.'.xls');
...
//titutlo
$objSheet->setCellValue('A4', 'MOVIMIENTO DE VENTAS DEL MES '.$meses[$mes].'');
...
$SQL_VENTAS=mysql_query("SELECT a.*, b.NOMBRE_PRODUCTO, c.FECHA_FACTURA FROM  detalle_boleta as a, producto as b, boleta as c WHERE a.COD_PRODUCTO=b.COD_PRODUCTO AND a.NRO_FACTURA=c.NRO_FACTURA AND MONTH(c.FECHA_FACTURA)='$mes' and  YEAR(c.FECHA_FACTURA)='$año' order by a.NRO_FACTURA  ",$conexion);
$numero=5;
